...
/* ═══ DESIGN TOKENS ═══ */
const T = {
  bg: { dark: '#0e0f14', light: '#f4f5f8', surface: '#fff', surfaceHover: '#fcfcfd', surfaceActive: '#f6f5f3' },
  ...
  shadow: { sm: '0 1px 2px rgba(0,0,0,0.04), 0 0 0 1px rgba(0,0,0,0.02)', md: '0 2px 12px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.03)', lg: '0 8px 32px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.03)', xl: '0 16px 48px rgba(0,0,0,0.1), 0 0 0 1px rgba(0,0,0,0.04)' },
      ...
      <div style={{
            ...
            return (
              <div
                ...
                style={{
                  ...
                  animation: `fadeUp 0.3s cubic-bezier(0.16,1,0.3,1) ${0.05 + i * 0.04}s both`,
                }}
              >
                {/* Number badge */}
                <div style={{
        ...
        {(allowCustom || onSkip) && (
          <div style={{
            ...
            animation: `fadeUp 0.3s cubic-bezier(0.16,1,0.3,1) ${0.05 + options.length * 0.04}s both`,
          }}>
          ...
          <div style={{
            ...
            {sampleRows.map((row, i) => (
              <div key={i} style={{
                ...
                <span style={{
                  flex: 2, fontFamily: "'JetBrains Mono', monospace",
...
function DotBackground({ isDark }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = dark ? "rgba(255,255,255,0.18)" : "rgba(150,145,135,0.25)";
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ width: 220, borderRight: "1px solid #eceef2", padding: "18px 16px" }}>
          ...
          {[
            ...
            <div key={f.n} style={{ padding: "5px 8px", borderRadius: 4, marginBottom: 2, background: "#f8f9fb", borderLeft: "3px solid #4f6df5" }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: "#4f6df5", fontWeight: 600 }}>{f.n}</div>
          ...
          {fields.map((f, i) => (
            <div key={f.n} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: i === currentIdx ? "#4f6df5" : "#3a3f4a", fontWeight: 600 }}>{f.n}</span>
        ...
        <div style={{ flex: 1, padding: "18px 22px" }}>
          {current ? (<>
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 700, color: "#1a1d23", marginBottom: 4 }}>{current.n}</div>
    ...
    <div style={{ width: "100%", maxWidth: 400, padding: "0 24px", animation: "fadeUp 0.4s ease" }}>
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 16px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.03)", overflow: "hidden" }}>
        <div style={{ padding: "16px 18px 14px" }}>
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 16, fontWeight: 700, color: "#1a1d23", marginBottom: 8 }}>{field.n}</div>
          {field.samples && (
            <div style={{ background: "#f8f9fb", borderRadius: 8, padding: "6px 10px", marginBottom: 12, border: "1px solid #eceef2" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {field.samples.map((s, i) => (
                  <div key={i} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#5a5f6e", padding: "1.5px 0", borderBottom: i < field.samples.length - 1 ? "1px solid rgba(0,0,0,0.03)" : "none" }}>{s}</div>
                ))}
              </div>
            </div>
          )}
    ...
    <div style={{
      ...
      {coreFields.map(f => (
        <div key={f.n} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
          ...
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#7a8194" }}>{f.n}</span>
        </div>
      ))}
      ...
      {fields.map((f, i) => {
        ...
        return (
          <div key={f.n} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
            ...
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: isCurrent ? "#4f6df5" : isDone ? "#7a8194" : "#b0b5c0", fontWeight: isCurrent ? 600 : 400 }}>{f.n}</span>
          </div>
        );
      })}
    </div>
...
function CoreFieldsEducation({ onAccept }) {
  ...
  // All columns with probability scores per role (higher = more likely to be that role)
  const allColumns = [
    { n: "case_id", unique: 91179, samples: ["PO-2024-00142", "PO-2024-00143", "PO-2024-00144", "PO-2024-00145", "PO-2024-00146"], prob: { case_id: 97, activity: 1, timestamp: 0 } },
    { n: "activity_name", unique: 9, samples: ["Created", "Free", "Approved", "Sent", "In Process"], prob: { case_id: 0, activity: 98, timestamp: 0 } },
    { n: "timestamp", unique: 624800, samples: ["2024-08-01 09:12", "2024-08-01 09:15", "2024-08-01 10:30"], prob: { case_id: 0, activity: 0, timestamp: 99 } },
    { n: "receiving_site", unique: 17, samples: ["PM1", "PN1", "PK3", "PL2", "PM2"], prob: { case_id: 12, activity: 5, timestamp: 0 } },
    { n: "purchase_office", unique: 28, samples: ["DE01", "DE02", "NO01", "SE01", "DE03"], prob: { case_id: 18, activity: 3, timestamp: 0 } },
    { n: "item_group", unique: 427, samples: ["MECH", "ELEC", "RAW", "PACK", "CHEM"], prob: { case_id: 8, activity: 15, timestamp: 0 } },
    { n: "selection_code", unique: 7, samples: ["A1", "B2", "C1", "A2", "B1"], prob: { case_id: 2, activity: 8, timestamp: 0 } },
    { n: "item_signal", unique: 8, samples: ["AUTO", "MANUAL", "null", "null", "null"], prob: { case_id: 0, activity: 4, timestamp: 0 } },
    { n: "created_date", unique: 365, samples: ["2024-08-01", "2024-08-02", "2024-08-03"], prob: { case_id: 0, activity: 0, timestamp: 22 } },
    { n: "modified_date", unique: 412, samples: ["2024-08-01 14:30", "2024-08-02 09:15"], prob: { case_id: 0, activity: 0, timestamp: 15 } },
  ];
  const dataColumns = allColumns.slice(0, 6); // for backward compat with steps 0-3
  const [expandedCol, setExpandedCol] = useState(null); // which alternative column is expanded
    ...
    <div style={{
      ...
      <div style={{ background: T.bg.surface, borderRadius: T.radius.lg, boxShadow: T.shadow.lg, overflow: "hidden", border: `1px solid ${T.border.light}` }}>
        <div style={{ padding: "28px 32px 24px" }}>
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
              {[
                ...
                <div key={ci} style={{
                  ...
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11.5, fontWeight: 700, color: col.color, letterSpacing: "-0.2px" }}>{col.n}</div>
          ...
          {/* Step 4: Three-column field selection with ranked alternatives */}
          {step === 4 && (
            <div style={{ animation: "snapIn 0.4s cubic-bezier(0.34,1.56,0.64,1)" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                {roles.map(r => {
                  const selectedCol = selections[r.key];
                  const selectedData = allColumns.find(c => c.n === selectedCol);
                  // Rank alternatives by probability for this role, exclude the selected one
                  const alternatives = allColumns
                    .filter(c => c.n !== selectedCol)
                    .sort((a, b) => (b.prob[r.key] || 0) - (a.prob[r.key] || 0))
                    .filter(c => (c.prob[r.key] || 0) > 0);
                    ...
                    <div key={r.key} style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <span style={{ fontSize: 14, color: r.color, lineHeight: 1 }}>{r.icon}</span>
                          <span style={{ fontSize: 12, fontWeight: 700, color: r.color }}>{r.label}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
                          <span style={{ fontSize: 10, color: T.text.muted }}>{r.desc}</span>
                          <span style={{ fontSize: 8, fontWeight: 700, color: T.text.subtle, textTransform: "uppercase", letterSpacing: "0.7px" }}>Confidence</span>
                        </div>
                      </div>

                      {/* Selected field — top, highlighted */}
                      {selectedData && (
                        <div style={{
                          padding: "10px 12px",
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                                <circle cx="7" cy="7" r="6" fill={r.color} />
                                <path d="M4 7l2 2 4-4" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                              </svg>
                              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: T.text.primary }}>{selectedData.n}</span>
                            </div>
                            <span style={{
                              fontSize: 10, fontWeight: 700, color: r.color,
                              background: `${r.color}15`, padding: "2px 7px", borderRadius: T.radius.pill,
                            }}>{selectedData.prob[r.key]}% match</span>
                          </div>
                          {/* Sample values */}
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                            {selectedData.samples.slice(0, 4).map((s, i) => (
                              <span key={i} style={{
                                fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5,
                                color: T.text.secondary, padding: "1px 5px",
                                background: T.bg.surfaceActive, borderRadius: 3,
                              }}>{s}</span>
                            ))}
                          </div>
                          <button onClick={() => setSelections(prev => ({ ...prev, [r.key]: null }))} style={{
                            marginTop: 6, fontSize: 10, color: T.text.muted, background: "none",
                      ...
                      {/* Alternative fields — scrollable list ranked by probability */}
                      <div style={{ maxHeight: 180, overflowY: "auto" }}>
                        {(!selectedCol ? allColumns.sort((a, b) => (b.prob[r.key] || 0) - (a.prob[r.key] || 0)) : alternatives).map((col, ci) => {
                          const prob = col.prob[r.key] || 0;
                          const isExpanded = expandedCol === `${r.key}-${col.n}`;
                          // Check if this column is already selected for another role
                          const usedByOther = Object.entries(selections).find(([k, v]) => v === col.n && k !== r.key);
                          const altBg = isExpanded ? T.bg.surfaceActive : ci % 2 === 0 ? "transparent" : "rgba(0,0,0,0.01)";

                          return (
                            <div key={col.n}
                              onClick={() => setExpandedCol(isExpanded ? null : `${r.key}-${col.n}`)}
                              style={{
                                padding: "7px 12px",
                                borderBottom: `1px solid ${T.border.light}`,
                                cursor: "pointer",
                              ...
                              onMouseEnter={e => { if (!isExpanded) e.currentTarget.style.background = T.bg.surfaceHover; }}
                              onMouseLeave={e => { if (!isExpanded) e.currentTarget.style.background = altBg; }}
                            >
                              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                <span style={{
                                  fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5,
                                  fontWeight: 500, color: T.text.primary,
                                }}>{col.n}</span>
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                  {usedByOther && (
                                    <span style={{ fontSize: 8, color: T.text.muted }}>{roles.find(x => x.key === usedByOther[0])?.label}</span>
                                  )}
                                  {/* Probability bar */}
                                  <div style={{ width: 40, height: 4, background: T.border.light, borderRadius: 2, overflow: "hidden" }}>
                                    <div style={{ width: `${prob}%`, height: "100%", background: prob > 50 ? r.color : prob > 10 ? T.text.muted : T.border.focus, borderRadius: 2 }} />
                                  </div>
                                  <span style={{
                                    fontSize: 9, fontWeight: 600, color: prob > 50 ? r.color : T.text.muted,
                                    minWidth: 26, textAlign: "right",
                                  }}>{prob}%</span>
                                </div>
                              </div>

                              {/* Expanded: show samples + select button */}
                              {isExpanded && (
                                <div style={{ marginTop: 6, animation: "fadeUp 0.2s ease" }}>
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                    <span style={{ fontSize: 9, color: T.text.muted }}>{col.unique.toLocaleString()} unique</span>
                                    {!usedByOther && (
                                      <button onClick={(e) => { e.stopPropagation(); assignRole(col.n, r.key); setExpandedCol(null); }}
                                        style={{
                                          marginLeft: "auto", padding: "3px 10px", fontSize: 10, fontWeight: 600,
                                          background: r.color, color: T.text.inverse, border: "none",
...
function StrategyAnimation({ step, mission }) {
  ...
  // No CSS transform zoom — instead, elements reposition via CSS transitions for reliable rendering
  ...
  return (
    <div style={{
      ...
      transform: isFading ? "scale(0.97) translateY(8px)" : "none",
      transition: "opacity 0.6s ease, transform 0.6s ease",
    }}>
      <div style={{
        background: T.bg.surface, borderRadius: T.radius.xl,
        boxShadow: T.shadow.lg, overflow: "hidden",
        border: `1px solid ${T.border.light}`,
        padding: "20px 24px",
      }}>
        <svg width="100%" viewBox="0 0 600 340" style={{ display: "block", minHeight: 280 }}>
          <defs>
            <filter id="nodeShadow"><feDropShadow dx="0" dy="1" stdDeviation="3" floodColor="#000" floodOpacity="0.07"/></filter>
            <filter id="glowBlue"><feDropShadow dx="0" dy="0" stdDeviation="8" floodColor={T.accent.blue} floodOpacity="0.3"/></filter>
          </defs>

          {/* ── STEP 1: Strategy node — top center ── */}
          {step >= 1 && (
            <g style={{ animation: "fadeUp 0.6s cubic-bezier(0.16,1,0.3,1)" }}>
              <rect x="245" y="15" width="110" height="38" rx="10" fill={T.text.primary} filter="url(#nodeShadow)" />
              <text x="300" y="39" textAnchor="middle" fill={T.text.inverse} fontSize="12" fontWeight="700" fontFamily={T.font}>Strategy</text>
            </g>
          )}

          {/* ── STEP 2: Three process branches ── */}
          {step >= 2 && (
            <>
              {/* Lines from strategy down */}
              <line x1="300" y1="53" x2="180" y2="100" stroke={isZoomed ? T.accent.blue : T.accent.blue} strokeWidth={isZoomed ? "2" : "1.5"} opacity={1} style={{ animation: "fadeUp 0.4s ease", transition: "all 0.8s ease" }} />
              <line x1="300" y1="53" x2="410" y2="95" stroke={isZoomed ? T.border.light : T.accent.orange} strokeWidth="1.5" strokeDasharray={isZoomed ? "4,4" : "none"} opacity={isZoomed ? 0.3 : 1} style={{ animation: "fadeUp 0.5s ease", transition: "all 0.8s ease" }} />
              <line x1="300" y1="53" x2="410" y2="145" stroke={isZoomed ? T.border.light : T.accent.green} strokeWidth="1.5" strokeDasharray={isZoomed ? "4,4" : "none"} opacity={isZoomed ? 0.3 : 1} style={{ animation: "fadeUp 0.6s ease", transition: "all 0.8s ease" }} />

              {/* Procurement node — grows when zoomed */}
              <g style={{ animation: "fadeUp 0.5s ease" }}>
                <rect x={isZoomed ? "100" : "120"} y={isZoomed ? "85" : "100"} width={isZoomed ? "160" : "120"} height={isZoomed ? "48" : "36"} rx="10"
                  fill={isZoomed ? T.accent.blue : T.bg.surface} stroke={T.accent.blue} strokeWidth={isZoomed ? "2.5" : "1.5"}
                  filter={isZoomed ? "url(#glowBlue)" : "url(#nodeShadow)"}
                  style={{ transition: "all 1s cubic-bezier(0.16,1,0.3,1)" }} />
                <text x={isZoomed ? "180" : "180"} y={isZoomed ? "114" : "123"} textAnchor="middle" fontSize={isZoomed ? "14" : "11"} fontWeight="600"
                  fill={isZoomed ? T.text.inverse : T.text.primary} fontFamily={T.font}
                  style={{ transition: "all 1s ease" }}>Procurement</text>
              </g>

              {/* Production node */}
              <g style={{ animation: "fadeUp 0.6s ease", opacity: isZoomed ? 0.2 : 1, transition: "opacity 0.8s ease" }}>
                <rect x="350" y="80" width="120" height="36" rx="10" fill={T.bg.surface} stroke={T.accent.orange} strokeWidth="1.5" filter="url(#nodeShadow)" />
                <text x="410" y="103" textAnchor="middle" fontSize="11" fontWeight="600" fill={T.text.primary} fontFamily={T.font}>Production</text>
              </g>

              {/* Logistics node */}
              <g style={{ animation: "fadeUp 0.7s ease", opacity: isZoomed ? 0.2 : 1, transition: "opacity 0.8s ease" }}>
                <rect x="350" y="130" width="120" height="36" rx="10" fill={T.bg.surface} stroke={T.accent.green} strokeWidth="1.5" filter="url(#nodeShadow)" />
                <text x="410" y="153" textAnchor="middle" fontSize="11" fontWeight="600" fill={T.text.primary} fontFamily={T.font}>Logistics</text>
              </g>
            </>
          )}

          {/* ── STEP 3: Mission label under procurement ── */}
          {step >= 3 && isZoomed && (
            <g style={{ animation: "fadeUp 0.6s ease" }}>
              <text x="180" y="152" textAnchor="middle" fontSize="9" fill={T.text.muted} fontFamily={T.font} fontStyle="italic">
                {(mission || "").length > 70 ? `"${(mission || "").slice(0, 67)}..."` : `"${mission || ""}"`}
              </text>
            </g>
          )}

          {/* ── STEP 4: Goal nodes branch down from procurement ── */}
          {step >= 4 && [
            { label: "Reduce throughput time", y: 180 },
            { label: "Lower rework rate", y: 220 },
            { label: "Improve data quality", y: 260 },
            { label: "Increase conformance", y: 300 },
          ].map((g, i) => (
            <g key={i} style={{ animation: `fadeUp ${0.2 + i * 0.1}s cubic-bezier(0.34,1.56,0.64,1)` }}>
              {/* Dashed connector */}
              <line x1="180" y1="133" x2="130" y2={g.y + 15} stroke={T.accent.blue} strokeWidth="1" opacity="0.25" strokeDasharray="3,3" />
              {/* Goal pill */}
              <rect x="60" y={g.y} width="145" height="30" rx="8" fill={T.bg.surface} stroke={T.accent.blue} strokeWidth="1.5" filter="url(#nodeShadow)" />
              {/* Number circle */}
              <circle cx="78" cy={g.y + 15} r="9" fill={`${T.accent.blue}12`} stroke={T.accent.blue} strokeWidth="0.7" />
              <text x="78" y={g.y + 19} textAnchor="middle" fontSize="9" fontWeight="700" fill={T.accent.blue} fontFamily={T.font}>{i + 1}</text>
              {/* Label */}
              <text x="95" y={g.y + 19} fontSize="9.5" fontWeight="500" fill={T.text.primary} fontFamily={T.font}>{g.label}</text>
            </g>
          ))}
        </svg>
      </div>
...
/* ═══ TARGET REVIEW CARD — bottom area, like field review ═══ */
function TargetReviewCard({ target, goal, index, total, onAccept, onAdjust }) {
  const [hovered, setHovered] = useState(null);
  return (
    <div style={{
      width: "100%", maxWidth: 600, padding: "0 24px",
      ...
      <div style={{
        ...
        <div style={{ padding: "14px 20px 10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: T.text.primary }}>{target.n}</div>
          <span style={{ fontSize: 11, color: T.text.muted }}>{index + 1} of {total}</span>
        </div>

        {/* Current → Target */}
        <div style={{ padding: "0 20px 14px", display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ flex: 1, textAlign: "center", padding: "10px 0", background: T.bg.surfaceActive, borderRadius: T.radius.sm }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: T.text.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 2 }}>Current</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: T.text.primary }}>{target.b}</div>
          </div>
          <svg width="20" height="12" viewBox="0 0 20 12"><path d="M2 6h16M14 2l4 4-4 4" stroke={T.accent.blue} strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
          <div style={{ flex: 1, textAlign: "center", padding: "10px 0", background: `${T.accent.blue}08`, borderRadius: T.radius.sm, border: `1.5px solid ${T.accent.blue}30` }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: T.accent.blue, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 2 }}>Target</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: T.accent.blue }}>{target.t}</div>
          </div>
        </div>

        {/* Why + Benchmark */}
        <div style={{ padding: "0 20px 14px" }}>
          <div style={{ fontSize: 12.5, color: T.text.secondary, lineHeight: 1.6, marginBottom: 8 }}>{target.why}</div>
          <div style={{ padding: "8px 12px", background: T.bg.surfaceActive, borderRadius: T.radius.sm, borderLeft: `3px solid ${T.text.muted}` }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: T.text.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 2 }}>Industry benchmark</div>
            <div style={{ fontSize: 11, color: T.text.secondary, lineHeight: 1.5 }}>{target.benchmark}</div>
          </div>
        </div>
        ...
        <div style={{ padding: "0 12px 14px" }}>
          <div
            ...
            style={{
              display: "flex", alignItems: "center", gap: 14,
              padding: "11px 14px", borderRadius: T.radius.sm,
              background: hovered === "accept" ? "rgba(0,0,0,0.02)" : "transparent",
              cursor: "pointer", transition: "all 0.15s ease",
              borderBottom: `1px solid ${T.border.light}`,
            }}
          >
            <div style={{ width: 28, height: 28, borderRadius: 7, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: T.bg.surfaceActive, color: T.text.muted, fontSize: 12, fontWeight: 600 }}>1</div>
            <span style={{ fontSize: 14, color: T.text.primary, fontWeight: 500 }}>Accept this target</span>
            {hovered === "accept" && <span style={{ fontSize: 11, color: T.text.subtle, marginLeft: "auto" }}>{"\u21B5"}</span>}
          </div>
          <div
            onClick={onAdjust}
            onMouseEnter={() => setHovered("adjust")}
            onMouseLeave={() => setHovered(null)}
            style={{
              display: "flex", alignItems: "center", gap: 14,
              padding: "11px 14px", borderRadius: T.radius.sm,
              background: hovered === "adjust" ? "rgba(0,0,0,0.02)" : "transparent",
              cursor: "pointer", transition: "all 0.15s ease",
            }}
          >
            <div style={{ width: 28, height: 28, borderRadius: 7, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: T.bg.surfaceActive, color: T.text.muted }}>
            ...
            <span style={{ fontSize: 14, color: T.text.muted }}>Skip this one</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
      {data.map((d, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ fontSize: 10, color: "#7a8194", width: 70, textAlign: "right", flexShrink: 0, fontFamily: "'JetBrains Mono', monospace" }}>{d.l}</div>
            ...
            {data.fieldAnnotations.map((f, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  fontFamily: "'JetBrains Mono', monospace", color: T.accent.blue, background: "rgba(79,109,245,0.07)",
...
/* ═══ GOALS TREE MAP — inline on canvas, HTML-based for proper text ═══ */
function GoalsTreeMap({ data, activeTargetIdx }) {
  if (!data) return null;
  ...
  return (
    <div style={{
      width: "100%", maxWidth: 760, padding: "0 16px",
      animation: "fadeUp 0.5s ease",
    }}>
      {/* Root node — mission */}
      <div style={{
        maxWidth: 420, margin: "0 auto 0",
        padding: "18px 28px",
        background: T.accent.blue,
        borderRadius: 16,
        textAlign: "center",
        boxShadow: `0 4px 24px rgba(79,109,245,0.25), 0 0 0 1px rgba(79,109,245,0.1)`,
      }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.text.inverse, letterSpacing: "-0.2px", marginBottom: 6 }}>
          {data.processName || "Process"}
        </div>
        <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.7)", lineHeight: 1.5, fontStyle: "italic" }}>
          "{data.mission || ""}"
        </div>
      </div>
      ...
      <div style={{ height: 40, position: "relative" }}>
        <svg width="100%" height="40" style={{ display: "block", overflow: "visible" }}>
          {goals.map((g, i) => {
            const hasTarget = !!g.b;
            const pct = (i + 0.5) / goals.length * 100;
            return (
              <line key={i}
                x1="50%" y1="0" x2={`${pct}%`} y2="40"
                stroke={hasTarget ? T.accent.blue : "rgba(160,155,145,0.3)"}
                strokeWidth={hasTarget ? "2" : "1.5"}
                strokeDasharray={hasTarget ? "none" : "5,4"}
      ...
      <div style={{
        ...
        {goals.map((g, i) => {
          const hasTarget = !!g.b;
          const isActive = i === activeTargetIdx;
          return (
            <div key={i} style={{
              position: "relative",
              padding: "14px 12px",
              background: isActive ? "rgba(255,255,255,0.97)" : hasTarget ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.75)",
              borderRadius: 12,
              border: isActive ? `2px solid ${T.accent.blue}` : hasTarget ? `1.5px solid ${T.accent.green}` : `1.5px solid rgba(160,155,145,0.2)`,
              boxShadow: isActive ? `0 2px 16px rgba(79,109,245,0.12)` : `0 1px 6px rgba(0,0,0,0.04)`,
              textAlign: "center",
              transition: "all 0.4s ease",
              animation: `fadeUp ${0.3 + i * 0.08}s cubic-bezier(0.34,1.56,0.64,1) both`,
            }}>
              {/* Active pulse dot */}
              {isActive && (
                <div style={{
                  position: "absolute", top: -5, left: "50%", transform: "translateX(-50%)",
                  width: 8, height: 8, borderRadius: "50%", background: T.accent.blue,
              ...
              <div style={{
                ...
                lineHeight: 1.35, marginBottom: 6, minHeight: 32,
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                {g.n}
              </div>

              {/* Target or current value */}
              {hasTarget ? (
                <div style={{
                  fontSize: 11, fontWeight: 600, color: T.accent.blue,
                  fontFamily: "'JetBrains Mono', monospace",
                  animation: "fadeUp 0.3s ease",
                }}>
                  {g.b} → {g.t}
...
const CANVAS_RESPONSES = [
  { match: /bottleneck|slow|delay/i, findingId: "bottleneck-lcp", text: "Last Confirmation Print is the primary bottleneck -- 35,092 cases with 5.7d average duration. The Approved to Last Confirmation transition alone adds 2.3d. I'd recommend investigating the confirmation approval rules and whether manual steps can be automated." },
  { match: /rework|loop|repeat/i, findingId: "rework-approved", text: "Two major rework patterns: (1) Approved self-loop -- 10,009 cases cycling back at 1.5d each, likely re-approvals after spec changes. (2) Blocked to Approved return -- 12,457 cases at 10.1h, suggesting blocked orders require re-approval. Together these add ~3.2 days to average throughput." },
  { match: /block/i, findingId: "blocked-volume", text: "24,435 cases pass through Blocked (68% of total). The Blocked self-loop (9,826 cases, 7.0h) suggests orders get stuck in repeated blocking cycles. The Blocked to Free return path is rare (15 cases) -- most blocked orders route back through Approved." },
  ...
  { match: /improv|fix|recommend|action|what.*do|next.*step/i, findingId: "site-variance", text: "Based on the context, I'd prioritize three actions: (1) Automate the confirmation print step -- 5.7d average is the single largest delay. (2) Investigate the Approved self-loop trigger -- 10,009 unnecessary re-approvals. (3) Add a fast-track path from Blocked to Last Confirmation Print for orders that resolve quickly." },
];
...
const CANVAS_FINDINGS = [
  { id: "bottleneck-lcp", title: "Confirmation print bottleneck", severity: "High", sevColor: "#d4685a",
    trigger: "bottleneck", goalLink: "Reduce throughput time",
    summary: "Last Confirmation Print adds 5.7d avg to every case. This single step accounts for over 40% of total throughput time.",
    recommendation: "Investigate confirmation approval automation. Consider rules-based auto-confirmation for standard orders." },
  { id: "rework-approved", title: "Approval rework loop", severity: "High", sevColor: "#d4685a",
    trigger: "rework", goalLink: "Lower rework rate",
    summary: "10,009 cases cycle through the Approved self-loop at 1.5d each. Combined with the Blocked→Approved return path (12,457 cases), rework adds ~3.2 days to average throughput.",
    recommendation: "Investigate re-approval triggers. Reduce specification change frequency. Add validation at order creation to prevent rework." },
  { id: "blocked-volume", title: "Excessive blocking rate", severity: "Medium", sevColor: "#e8a040",
    trigger: "block", goalLink: "Lower rework rate",
    summary: "68% of all cases pass through Blocked. The self-loop (9,826 cases, 7.0h each) suggests orders get stuck in repeated blocking cycles.",
    recommendation: "Analyze blocking reasons by vendor and product category. Create fast-track unblocking for common resolution patterns." },
  { id: "site-variance", title: "Cross-site performance gap", severity: "Medium", sevColor: "#e8a040",
    trigger: "improv", goalLink: "Reduce throughput time",
...
function CanvasView({ storedFindings, setStoredFindings, onStartImprovement }) {
  ...
  const [expandedFinding, setExpandedFinding] = useState(null); // which finding ID is expanded
  const [showGoalsPanel, setShowGoalsPanel] = useState(true); // goals panel toggle — open by default on entry
  const [guideStep, setGuideStep] = useState(0); // 0=show goals, 1=highlight bottleneck, 2=clicked, 3=chatted, 4=done
  ...
  const MUTTERS = [
    "Click the red node →",
    "5.7 days... let's investigate",
    "Your throughput goal needs this",
    "I found something here",
    "This bottleneck matters",
  ];
  useEffect(() => {
  ...
  useEffect(() => {
    // Step 0: Goals panel is already open (default true)
    // Step 1: After 3s, close goals panel and highlight bottleneck
    const t1 = setTimeout(() => {
      setGuideStep(1);
    }, 3000);
    // Step 2: Show initial guidance message after 4.5s
    const t2 = setTimeout(() => {
      setChatMessages([{ role: "ai", text: "Your goal is to reduce throughput time from 18.8d to 14d. I've highlighted the biggest bottleneck \u2014 Last Confirmation Print at 5.7d average. Click it to explore." }]);
    }, 4500);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, []);

  // Proactively propose a finding after user clicks the bottleneck node (guideStep 2)
  useEffect(() => {
    if (guideStep !== 2) return;
    const t = setTimeout(() => {
      setChatMessages(prev => [...prev, { role: "ai", text: "I can see this node is the primary bottleneck \u2014 5.7 days average across 35,092 cases. Let me check if there's a pattern here worth flagging..." }]);
      setTimeout(() => {
        triggerFinding("bottleneck-lcp");
      }, 2000);
    }, 1200);
    return () => clearTimeout(t);
  }, [guideStep]);
  ...
  const addContext = (item) => {
    ...
    setChatMessages(prev => [...prev, { role: "ai", text: `Added "${item.label}" to context. Ask me anything about it.` }]);
  };

  const triggerFinding = (findingId) => {
    const finding = CANVAS_FINDINGS.find(f => f.id === findingId);
    if (!finding) return;
    if (chatMessages.some(m => m.type === "finding" && m.id === findingId)) return; // already shown
    setTimeout(() => {
      setChatMessages(prev => [...prev, { role: "ai", text: `I've identified a pattern worth investigating.` }]);
      setTimeout(() => {
        setChatMessages(prev => [...prev, { role: "ai", type: "finding", ...finding }]);
      }, 800);
    }, 1500);
  };
  ...
  const sendChat = () => {
    ...
    setTimeout(() => {
      ...
      setAiTyping(false);
      if (match && match.findingId) triggerFinding(match.findingId);
      else if (newCount >= 3 && !chatMessages.some(m => m.type === "finding")) triggerFinding("bottleneck-lcp");
    }, 800);
  };

  const storeFinding = (findingId) => {
    setStoredFindings(prev => prev.includes(findingId) ? prev : [...prev, findingId]);
    // Remove the finding card from chat messages
    setChatMessages(prev => prev.filter(m => !(m.type === "finding" && m.id === findingId)));
    // Open goals panel to show the finding under its goal
    setShowGoalsPanel(true);
  };

  const keepExploring = () => {
    setChatMessages(prev => [...prev, { role: "ai", text: "Let's keep digging. Try clicking different nodes or ask me about specific patterns." }]);
  };
  ...
  const handleCanvasMouseDown = (ev) => {
    if (ev.target !== ev.currentTarget && !ev.target.closest("[data-pan]")) return;
    setIsPanning(true);
    panStart.current = { x: ev.clientX, y: ev.clientY, ox: panOffset.x, oy: panOffset.y };
  };
    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", animation: "canvasEnter 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button onClick={() => setShowGoalsPanel(!showGoalsPanel)} style={{
            padding: "4px 10px", borderRadius: 6, fontSize: 10, fontWeight: 600,
            background: showGoalsPanel ? T.accent.blue : "#f0f2f5",
            color: showGoalsPanel ? T.text.inverse : T.text.secondary,
            border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 4,
            transition: "all 0.2s ease",
          }}>
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
              <circle cx="8" cy="4" r="3" stroke="currentColor" strokeWidth="1.3" fill="none"/>
              <line x1="8" y1="7" x2="8" y2="10" stroke="currentColor" strokeWidth="1.3"/>
              <line x1="4" y1="13" x2="8" y2="10" stroke="currentColor" strokeWidth="1.3"/>
              <line x1="12" y1="13" x2="8" y2="10" stroke="currentColor" strokeWidth="1.3"/>
            </svg>
            Goals
            {storedFindings.length > 0 && (
              <span style={{ padding: "1px 5px", borderRadius: 8, fontSize: 9, fontWeight: 700, background: showGoalsPanel ? "rgba(255,255,255,0.2)" : T.accent.blue, color: T.text.inverse }}>{storedFindings.length}</span>
            )}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        {/* Process DAG — centered, draggable */}
        <div data-pan="true" onClick={dismissPopups}
          ...
          style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1, cursor: isPanning ? "grabbing" : "grab" }}>
          <div style={{ position: "relative", width: 720, height: 700, animation: "canvasEnter 0.6s cubic-bezier(0.16,1,0.3,1) 0.3s both", transform: `translate(${panOffset.x}px, ${panOffset.y}px)` }}>
            ...
            {nodes.map((n, i) => {
              ...
              return (
                <div key={`n${i}`} onMouseEnter={() => setHoveredNode(i)} onMouseLeave={() => setHoveredNode(null)}
                  ...
                  {/* Guide highlight — pulsing ring on bottleneck */}
                  {n.bottleneck && guideStep === 1 && (
                    <div style={{
                      position: "absolute", top: -4, left: -4, right: -4, bottom: -4,
                      borderRadius: 14, border: `2.5px solid ${T.accent.red}`, opacity: 0.6,
                      pointerEvents: "none", animation: "findingGlow 2s ease infinite",
                    }} />
                  )}
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 20, padding: "10px 12px", background: "linear-gradient(135deg, rgba(79,109,245,0.06), rgba(79,109,245,0.02))", borderRadius: T.radius.sm, border: `1px solid ${T.border.accent}` }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: T.accent.blue, textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: 8 }}>Key Insights</div>
            {[
              { icon: "\u23F1", label: "Bottleneck", value: "5.7d avg", detail: "Last Confirmation Print", color: T.accent.red },
              { icon: "\uD83D\uDD04", label: "Rework rate", value: "52%", detail: "Approved self-loop", color: T.accent.orange },
              { icon: "\uD83D\uDEAB", label: "Block rate", value: "68%", detail: "Cases through Blocked", color: T.accent.orange },
              { icon: "\uD83D\uDCCA", label: "Site gap", value: "4.4x", detail: "PM1 vs PN1", color: T.accent.blue },
            ].map((ins, i) => (
              <div key={i} style={{
                display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", marginBottom: 4,
                background: T.bg.surface, borderRadius: T.radius.xs, border: `1px solid ${T.border.light}`,
                cursor: "pointer", transition: "all 0.15s",
              }}
                onMouseEnter={e => e.currentTarget.style.borderColor = ins.color}
                onMouseLeave={e => e.currentTarget.style.borderColor = T.border.light}
                onClick={() => {
                  addContext({ type: "insight", label: ins.label, id: `insight-${ins.label}` });
                }}
              >
                <span style={{ fontSize: 12 }}>{ins.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: T.text.primary }}>{ins.label}: <span style={{ color: ins.color }}>{ins.value}</span></div>
                  <div style={{ fontSize: 9, color: T.text.muted }}>{ins.detail}</div>
                </div>
                <SparklesIcon size={10} color={T.text.subtle} />
              </div>
            ))}
          </div>
        ...
        {/* Goals panel — floating overlay */}
        {showGoalsPanel && (
          <div style={{
            position: "absolute", top: 48, right: 16, width: 320,
            ...
            <div style={{ padding: "14px 16px 10px", borderBottom: `1px solid ${T.border.light}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: T.text.primary }}>Mission & Goals</span>
              <button onClick={() => setShowGoalsPanel(false)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, color: T.text.muted, padding: "2px" }}>×</button>
            </div>
            <div style={{ padding: "12px 16px", overflowY: "auto" }}>
              {/* Compact mission */}
              <div style={{ padding: "10px 12px", background: T.accent.blue, borderRadius: T.radius.sm, marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: T.text.inverse, marginBottom: 3 }}>Purchase-to-Receive</div>
                <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.7)", lineHeight: 1.4, fontStyle: "italic" }}>"To have an efficient purchasing process that is uniform across all sales offices."</div>
              </div>
              {/* Goals list with findings nested under linked goals */}
              {[
                { n: "Reduce throughput time", b: "18.8d", t: "14d" },
                { n: "Lower rework rate", b: "52%", t: "35%" },
                { n: "Improve data quality", b: "50/100", t: "70+" },
                { n: "Increase conformance", b: "1,175", t: "<400" },
              ].map((g, i) => {
                const linkedFindings = storedFindings.map(fid => CANVAS_FINDINGS.find(cf => cf.id === fid)).filter(f => f && f.goalLink === g.n);
                return (
                <div key={i} style={{ padding: "8px 10px", borderRadius: T.radius.sm, marginBottom: 4, border: `1px solid ${linkedFindings.length > 0 ? T.border.accent : T.border.light}`, background: linkedFindings.length > 0 ? "rgba(79,109,245,0.03)" : T.bg.surface }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: T.text.primary }}>{g.n}</span>
                    <span style={{ fontSize: 10, fontWeight: 600, color: T.accent.blue, fontFamily: "'JetBrains Mono', monospace" }}>{g.b} → {g.t}</span>
                  </div>
                  {linkedFindings.map(f => (
                    <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5, padding: "4px 8px", borderRadius: 4, background: `${f.sevColor}08`, border: `1px solid ${f.sevColor}18`, animation: "fadeUp 0.3s ease" }}>
                      <div style={{ width: 5, height: 5, borderRadius: "50%", background: f.sevColor, flexShrink: 0 }} />
                      <span style={{ fontSize: 9.5, fontWeight: 500, color: T.text.secondary, flex: 1 }}>{f.title}</span>
                      <span style={{ fontSize: 8, fontWeight: 600, padding: "1px 5px", borderRadius: 3, background: `${f.sevColor}12`, color: f.sevColor }}>{f.severity}</span>
                    </div>
                  ))}
                </div>
                );
              })}
              {storedFindings.length >= 2 && (
                <div style={{
                  padding: "10px 12px", marginTop: 8,
                  background: `${T.accent.blue}08`, borderRadius: T.radius.sm,
                  border: `1px solid ${T.border.accent}`,
                  textAlign: "center",
                }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: T.accent.blue, marginBottom: 4 }}>Ready to improve?</div>
                  <div style={{ fontSize: 9.5, color: T.text.muted, marginBottom: 8 }}>You have enough findings to start building an improvement plan.</div>
        ...
        {/* No blob overlay — process map is the hero */}

      {/* Canvas chat — floating modal overlay on top of canvas */}
        <div style={{
          position: "absolute", bottom: 48, left: "50%", transform: "translateX(-50%)",
          width: "100%", maxWidth: 540, zIndex: 30,
          padding: "0 24px",
          pointerEvents: "none",
        }}>
          <div style={{
          ...
          {/* AI response + finding cards */}
          {chatMessages.length > 0 && (() => {
            // Show the last AI message AND any pending finding
            const lastAi = [...chatMessages].reverse().find(m => m.role === "ai" && !m.type);
            const pendingFinding = [...chatMessages].reverse().find(m => m.type === "finding" && !storedFindings.includes(m.id));
            return (
              <div style={{ marginBottom: 10 }}>
                {lastAi && (
                  <div style={{
                    padding: "10px 14px", marginBottom: pendingFinding ? 8 : 0,
                    background: T.bg.light, borderRadius: T.radius.md,
                    animation: "fadeUp 0.3s ease",
                  }}>
                ...
                {pendingFinding && (
                  <div style={{
                    ...
                    background: "linear-gradient(135deg, #fafaff, #f0f2ff)",
                    border: `1.5px solid ${T.border.accent}`,
                    boxShadow: T.shadow.md,
                    animation: "findingGlow 2s ease infinite",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 4, background: `${pendingFinding.sevColor}15`, color: pendingFinding.sevColor }}>{pendingFinding.severity}</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: T.text.primary }}>{pendingFinding.title}</span>
                    </div>
                    <div style={{ fontSize: 12, color: T.text.secondary, lineHeight: 1.6, marginBottom: 8 }}>{pendingFinding.summary}</div>
                    {expandedFinding === pendingFinding.id && (
                      <div style={{ fontSize: 11.5, color: T.accent.blue, lineHeight: 1.6, padding: "8px 0", borderTop: `1px solid ${T.border.accent}`, marginBottom: 4 }}>
                        <div style={{ fontWeight: 600, marginBottom: 3 }}>Recommendation</div>
                        {pendingFinding.recommendation}
                      </div>
                    )}
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <button onClick={() => setExpandedFinding(expandedFinding === pendingFinding.id ? null : pendingFinding.id)}
                        style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: T.accent.blue, fontWeight: 600, padding: 0 }}>
                        {expandedFinding === pendingFinding.id ? "Hide details" : "View details"}
                      </button>
                      <div style={{ flex: 1 }} />
                      <button onClick={() => storeFinding(pendingFinding.id)}
                        style={{ padding: "5px 14px", borderRadius: T.radius.sm, background: T.accent.blue, color: T.text.inverse, border: "none", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                        Store finding
                      </button>
                      <button onClick={keepExploring}
                        style={{ padding: "5px 14px", borderRadius: T.radius.sm, background: T.bg.surfaceActive, color: T.text.secondary, border: `1px solid ${T.border.light}`, fontSize: 11, fontWeight: 500, cursor: "pointer" }}>
                        Keep exploring
                      </button>
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center", marginBottom: 8 }}>
              {[
                ...
                <button key={qi} onClick={() => {
                  ...
                  setAiTyping(true);
                  setTimeout(() => {
                    const match = CANVAS_RESPONSES.find(r => r.match.test(q));
                    setChatMessages(prev => [...prev, { role: "ai", text: match ? match.text : CANVAS_DEFAULT_RESPONSE }]);
                    setAiTyping(false);
                    if (match && match.findingId) triggerFinding(match.findingId);
                  ...
                  style={{
                    ...
                    background: T.bg.light, color: T.accent.blue,
                    border: `1px solid ${T.border.accent}`,
                    borderRadius: T.radius.pill, cursor: "pointer", fontFamily: T.font,
                    transition: "all 0.15s",
                  }}
                  onMouseEnter={e => { e.currentTarget.style.background = T.accent.blue; e.currentTarget.style.color = T.text.inverse; }}
                  onMouseLeave={e => { e.currentTarget.style.background = T.bg.light; e.currentTarget.style.color = T.accent.blue; }}
                >{q}</button>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input value={chatInput} onChange={(e) => setChatInput(e.target.value)}
              ...
              style={{
                ...
                background: T.bg.light, color: T.text.primary,
                boxShadow: "none",
              }}
              onFocus={e => { e.target.style.borderColor = T.border.focus; e.target.style.boxShadow = `0 0 0 3px rgba(0,0,0,0.03)`; }}
              onBlur={e => { e.target.style.borderColor = T.border.light; e.target.style.boxShadow = "none"; }}
            />
            <button onClick={sendChat} style={{
              padding: "10px 18px", fontSize: 13, fontWeight: 600,
              fontFamily: T.font, background: T.text.primary, color: T.text.inverse,
              border: "none", borderRadius: T.radius.md, cursor: "pointer",
            }}
              onMouseEnter={e => e.currentTarget.style.background = "#2d3340"}
              onMouseLeave={e => e.currentTarget.style.background = T.text.primary}
            >Send</button>
          </div>
          </div>
        </div>

      {/* Close full-width canvas area */}
      </div>
...
const steps = [
  ...
  { id: "context", label: "Process context", phases: ["fact-gathering", "strategy-animation", "mission-review"] },
                ...
                {fieldProgress.fields.map((f, fi) => {
                  ...
                  return (
                    <div key={f.n} style={{ display: "flex", alignItems: "center", gap: 5, padding: "2px 0" }}>
                      ...
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: isActive ? "#4f6df5" : fDone ? "#7a8194" : "#b0b5c0", fontWeight: isActive ? 600 : 400 }}>{f.n}</span>
                    </div>
                  );
                })}
                {i < steps.length - 1 && (
...
export default function App() {
  ...
  const showDataChoice = useCallback(() => {
    showChoice({
  ...
  const advanceFactQuestion = (currentIdx) => {
    ...
    if (next < factQs.length) {
      ...
      // All questions done — show strategy animation BEFORE mission proposal
      setBlobText("Excellent. Now let me show you why what comes next is so important.");
      setActiveChoice(null);
      setTimeout(() => showStrategyThenMission(), 1200);
    }
  };

  // Strategy animation → then mission proposal
  const showStrategyThenMission = () => {
    setPhase("strategy-animation");
    setStrategyStep(0);
    setBlobText("");

    setTimeout(() => {
      setBlobText("Strategy flows through processes.");
      setStrategyStep(1);
    }, 800);

    setTimeout(() => {
      setBlobText("Your process is part of a larger value chain.");
      setStrategyStep(2);
    }, 3000);

    setTimeout(() => {
      setBlobText("Every process needs a mission — a clear north star for improvement.");
      setStrategyStep(3);
    }, 5500);

    setTimeout(() => {
      setBlobText("Goals make the mission actionable. Let me propose yours.");
      setStrategyStep(4);
    }, 8500);
  ...
  const acceptFields = () => {
    ...
    setTimeout(() => {
      ...
      // Step 3: Findings intro after 7s — list appears
      setTimeout(() => {
      ...
      // Blob encourages user to explore after 10s (extended to let user read)
      setTimeout(() => {
      ...
      // Step 4: Auto-expand first finding after 13s
      setTimeout(() => {
      ...
      // Step 5: Auto-collapse findings after 16s
      setTimeout(() => {
        setDqNarrativeStep(5);
      }, 16000);

      // Step 6: Wrap up after 18s
      setTimeout(() => {
  ...
  const approveDoc = () => {
    setDocVisible(false);
    setBlobText("The process context is saved. Now let's turn that mission into concrete goals.");
    setPhase("pre-goals");
    setTimeout(() => {
      setBlobText("I've identified 4 improvement goals based on your data — each one supports your mission. Select the ones that matter most.");
      setTimeout(() => { setPanel("goals"); setPhase("goals"); }, 600);
    }, 1500);
  };
  ...
  const acceptGoals = (selectedIndices) => {
    ...
    setGoalsDoc({
      processName: docData?.processName || "Purchase-to-Receive",
    ...
    setTargetIdx(0);
    setAcceptedTargets([]);
    setBlobText("Goals confirmed! Now I'll suggest a target for each one. Review them below — accept or adjust.");
    setPhase("targets");
  };
  ...
  const skipGoals = () => {
    ...
    setTimeout(() => {
      setBlob("waiting");
      setBlobText("No problem — we can come back to goals later. What would you like to do?");
      setPhase("junction");
    }, 600);
  };
  ...
  const acceptSingleTarget = () => {
    ...
    if (nextIdx < activeTargets.length) {
      ...
      setBlobText("All targets set! What would you like to do next?");
      setTimeout(() => { setPhase("junction"); }, 800);
    }
  };
  ...
  const skipSingleTarget = () => {
    ...
    if (nextIdx < activeTargets.length) {
      ...
      setBlobText("Targets done! What would you like to do next?");
      setTimeout(() => { setPhase("junction"); }, 800);
    }
  };
  ...
  const jumpTo = (step) => {
    ...
    if (step.id === "data") {
      ...
      setBlobText("What would you like to do next?");
      setPhase("junction");
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: isDark ? "#0e0f14" : "#f5f4f2", transition: "background 1.5s ease", display: "flex", flexDirection: "column", fontFamily: "'DM Sans', -apple-system, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative", ...(phase === "tour" || phase === "canvas" || phase === "improvement" ? { display: "none" } : {}) }}>
        ...
        {/* GOALS TREE MAP — moved to scroll lane, rendered inline below blob */}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", overflow: "hidden", position: "relative", marginRight: showProcessTeaser && !isThink && !hasDoc ? 352 : 0, transition: canvasTransitionPhase === "exit" ? "opacity 0.5s, transform 0.5s" : "margin 0.5s cubic-bezier(0.16,1,0.3,1)", opacity: canvasTransitionPhase === "exit" ? 0 : 1, transform: canvasTransitionPhase === "exit" ? "translateY(-40px)" : "none" }}>
            ...
            {(isThink || !hasStarted) ? (
              ...
              <>
                ...
                {/* Goals tree map — inline on canvas during goals/targets/junction */}
                {hasGoalsDoc && (phase === "goals" || phase === "targets" || phase === "junction" || phase === "pre-goals") && (
                  <GoalsTreeMap data={goalsDoc} activeTargetIdx={phase === "targets" ? targetIdx : -1} />
                )}

                {/* Junction panel — below tree map, inline */}
                {phase === "junction" && (
                  <div style={{ width: "100%", maxWidth: 520, padding: "16px 24px 0", animation: "fadeUp 0.4s ease" }}>
                    <JunctionPanel goals={goalsDoc?.goals || []} onSelect={(id) => {
                      setPanel(null);
                      if (id === "tour") {
                        setBlobText("");
                        setBgLight(0);
                ...
                {/* Review document — scrollable card in center */}
                {isReview && hasDoc && (
                  <div style={{
                      ...
                      <div style={{ flex: 1, overflowY: "auto", padding: "14px 20px 16px" }}>
                        ...
                        {docData.fieldAnnotations && docData.fieldAnnotations.length > 0 && <div style={{ marginBottom: 14 }}>
                          ...
                          {docData.fieldAnnotations.map((f, i) => (<div key={i} style={{ padding: "6px 10px", borderRadius: 5, marginBottom: 3, background: T.bg.surfaceActive, borderLeft: `3px solid ${T.accent.blue}` }}>
                            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: T.text.secondary, fontWeight: 600 }}>{f.field}</div>
      ...
      {phase !== "tour" && phase !== "canvas" && phase !== "junction" && (
        <div style={{
          ...
          {/* Target review — in bottom area like field review */}
          {phase === "targets" && activeTargets[targetIdx] !== undefined && !activeChoice && !refineState && (
            <TargetReviewCard
              key={targetIdx}
              target={ALL_TARGETS[activeTargets[targetIdx]]}
              goal={goalsDoc?.goals?.[targetIdx]}
              index={targetIdx}
              total={activeTargets.length}
              onAccept={acceptSingleTarget}
              onAdjust={skipSingleTarget}
            />
          )}

          {/* Default text input — only when no widget is active and user has made first choice */}
          {!activeChoice && !refineState && !(phase === "fields") && !isReview && phase !== "intro" && phase !== "data" && phase !== "strategy-animation" && phase !== "targets" && (
